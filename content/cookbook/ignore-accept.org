...
included,  respectively,  in the NONMEM data set.  A list has the
format:
#+begin_src perl
 cond,cond,cond,...
#+end_src
...
#+end_src
If the expressions are simple, this  may  be  done  by  negating  both sides:
#+begin_src f90
...
#+end_src
This  workaround cannot handle continuous values of A or  more complicated conditions.
...
NONMEM  run  using  the  $TABLE  record option "EXCLUDE_BY list".  If
any variable in list is non-0, the record is excluded from the table
file probe.tab.   The  $DATA  ACCEPT/IGNORE  option is not needed.
Because probe.tab does not exist when NM-TRAN processes the second

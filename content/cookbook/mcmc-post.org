...
the draw files for future csv parsing. So if one builds the utility
and applies it to the .ext file
#+begin_src bash
...
#+end_src
The new ~w15.ext.csv~ file would contain the original draw but with
title and summary lines commented out:
#+INCLUDE: "/examples/nuts-post/w15_head_tail.ext.csv" src r
so that one can read it in R:
#+begin_src r
...
Now we can postprocess the draws using R package [[https://mc-stan.org/posterior/index.html][posterior]]. We first
read in the ~data~ object as draws, and we skip the warmup (burn-in)
draws, the ones with ITERATION < 0.
#+begin_src r
  library(posterior)
...
#+end_src
We can see that the standard deviation is exactly what is reported on
...
visulizes the correlation between THETAs. We can see that the
...
contains the draws of PHIs and ETAs for each of the 100 subjects. The
posterior package provides many functions to manipulate draws. For
example, we can extract the individual parameters for subject 1, and
combine them with the population parameter draws.
#+begin_src r
  library(dplyr)
  d1 <- bind_draws(d_ind |> filter(SUBJECT_NO==1) |> select(-"MCMCOBJ"),
                   d |> select(-"MCMCOBJ"),
                   along='variable')
#+end_src

...
and summary lines:
#+begin_src r
  data <- read.csv("w15.ext", skip=1)
  data <- data[data$ITERATION>-1000000000, ]
#+end_src
Note that we filtered summary and diagnostic lines in that the [[file:/docs/reference-manual/files/#raw-output][raw output
files]], indicated by ~ITERATION=-100000000x~.

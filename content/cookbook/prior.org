...
By default, THETAS priors follow multivariate normal;
OMEGAS and SIGMAS priors follow Inverse Wishart distribution.
Alternatively,  a residual variance (in square root form)
may be used for THETA (a sigma-like THETA parameters is set up in
...
A ~$PRIOR NWPRI~ statement such as
#+begin_src perl
...
specifies the following items.
  ...
  - NEPP=number of SIGMAs with prior (NM73).

NM73+ provides following records for prior specification:
...
* Prior specification before NM73
Internally the prior information are stored along with ~THETA~, ~OMEGA~, ~SIGMA~:
- the ~$THETA~ records list the parameters, in order, the following:
...
Therefore one can also specify THETA, OMEGA, and
SIGMA priors in the following way, particularly for earlier versions.
#+begin_src perl
$THETA 2.0 2.0 4.0 4.0 ; Initial THETAs
...
To form a composite average set of prior parameters from multiple
studies, use ~$PRIOR NWPRI ... NEXP=n~, where ~n~ is the number of experiments.  Then add

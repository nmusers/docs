...
As an alternative to using EVID, CMT and RATE to specify infusion
doses, one can encode such information in $PK block. Consider
#+begin_src perl
 $PROBLEM  NETILMICIN
...
#+end_src
$INPUT and $DATA specifies two types of doses, intravenous (IV) and intramuscular (IM).

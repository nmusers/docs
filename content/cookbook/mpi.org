...
processors, so that MPI runs can be executed /locally/ to speed up
...
such as by [[https://learn.microsoft.com/en-us/message-passing-interface/microsoft-mpi][Microsoft MPI]], [[https://www.open-mpi.org/][OpenMPI]], and [[https://www.mpich.org/][MPICH]]. Note that NONMEM
contains libraries for Microsoft MPI (MSMPI), so users need only install
the MSMPI executable but not the SDK.
...
2. Assume NONMEM is located at ~/path/to/nonmem~, to build a model at ~/path/to/model/model-name.ctl~, in the build directory, do
   ...
   #+end_src
   enables MSMPI build, no ~MPI_HOME~ option needed.
...
script [[file:/docs/user-guide/nmtran/nmfe/#headline-10][here]].

The ~nodes=~ option of the ~nmf2~ script enable MPI build and run. It

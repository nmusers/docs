...
\frac{dA(t)}{dt} = K^TA(t)
$$
with \(A(t)\) an \(n\)-vector, including the [[file:/docs/user-guide/nmtran/data/#cmt][output compartment]], and
the rate coefficient matrix \(K\) an \(n\times n\) matrix.
...
- Basic PK parameters: ~Kij~: \(K_{ij}\), the rate constant compartment \(i\) to
        \(j\). In particular, with \(n\) for the output compartment,
        ~Kin~ can use alias ~Ki0~. One can use separator ~T~ to
        disambiguate ~i~ and ~j~: ~K1T12~ for \(K_{1,12}\), and
        ~K11T2~ for \(K_{11,2}\).
- Additional PK parameters (for each compartment ~i~ in the system):
      + ~Si~ - Scale for the i'th compartment;
      + ~S0~ - alias for the output compartment scaling ~Sn~.

- For each dosable compartment ~i~ in the system:
      + ~Fi~ - Bioavailability for the i'th compartment;
      + ~Ri~ - Rate for y compartment;
      + ~Di~ - Duration for the i'th compartment;
      + ~ALAGi~ - Absorption lag for the i'th compartment.

- Other additional PK parameters:
      + ~FO~ - Output fraction (with alias ~Fn~, ~F0~)
      + ~XSCALE~ - Time scale
      + ~MTIME(i)~ - Model event times
...
- ADVAN14 uses CVODES from the LLNL SUNDIALS  system  for  ordinary
  ...
  ADVAN14 also has a root-finding algorithm. The user may choose to
...
- ADVAN18 uses the DDE_SOLVER delay differential equation solver. See

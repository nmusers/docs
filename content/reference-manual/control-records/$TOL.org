...
- $TOL cannot be present if the $SUBROUTINES record includes any of  the corresponding options.
...
Left-hand quantities: these are identical to the left-hand variables that may be set in subroutine TOL.  If subscript "(I)" is omitted, it defaults to 1.
...
Right-hand quantities: Integers, e.g., NRD(1)=4

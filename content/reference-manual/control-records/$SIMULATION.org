...
Unless NOPREDICTION is used (see below), the simulated observation is
the quantity to which the Y variable (with NMTRAN abbreviated code) or
F variable (with  a  user-supplied  PRED  or ERROR  routine)  is set,
using the parameter values given as initial estimates (from $THETA,
$OMEGA, $SIGMA). Initial estimates must be given for all parameters.

...
R^{-1}\), with $R$ being the Hessian matrix and S the
cross-product  gradient matrix.
- ~MATRIX=R~: use \(2R^{-1}\). This gives SE estimates more consistent
  with other statistical softwares. This option should not be used with option SPECIAL.
- ~MATRIX=S~: use \(4S^{-1}\).

The option does not Covariance Step when ~METHOD=BAYES~ or ~METHOD=NUTS~.

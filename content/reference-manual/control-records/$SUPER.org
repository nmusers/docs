...
* $SUPER
 Identifies the start of an optional NONMEM Superproblem.
...
*** PRINT
      NONMEM  printout displaying the input information for each of the
      problems of the superproblem will be generated during all  iterations.

 To define a sequence of problems as a superproblem, precede the $PROBLEM record of the first problem of the sequence with a $SUPER  record.

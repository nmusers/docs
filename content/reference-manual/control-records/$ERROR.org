...
Certain  variables  in  FORTRAN  Modules  can  be  used.   (See @@html:<a href="/docs/reference-manual/api/variables/">Variables in modules</a>@@). The following are of particular interest.
  + A(n)  (Amount in compartment n. See @@html:<a href="/docs/reference-manual/api/variables/#state-vec-a">State Vector A</a>@@). If there is no verbatim code and no explicit use  of  a  subscripted variable  A in the $ERROR block, then the symbol A can be used as a data item label or as a name of an ERROR-defined item.
...
- CALLFL=1: Call once per individual record.
  ...
  (See @@html:<a href="/docs/reference-manual/abbr/#calling_protocol">CALLFL</a>@@).

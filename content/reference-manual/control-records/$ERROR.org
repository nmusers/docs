...
Certain  variables  in  FORTRAN  Modules  can  be  used. (See Variables in modules) The following are of particular interest.
  + A(n)  (Amount in compartment n.)  (See State Vector A). If there
    is no verbatim code and no explicit use  of  a  subscripted
    variable  A in the $ERROR block, then the symbol A can be used as
    a data item label or as a name of an ERROR-defined item.

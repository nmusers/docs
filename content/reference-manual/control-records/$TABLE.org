...
With NONMEM 7.4, a symbolic label specified in $ABBR REPLACE  may be
listed in $TABLE. For example:
#+begin_src perl
...
Reserved  positions of MODULE NMPRD4 (See [[file:/docs/reference-manual/control-records/abbrev/][$ABBREVIATED]]).  COM(k) or :k
...
Synonyms  may  be  defined on either the $TABLE or $SCATTER record for
 special items PRED, RES, WRES; for PRED-defined  items;  for  NM-TRANdefined items; and for reserved positions of MODULE NMPRD4.
...
#+end_src
 ...
 previous versions of NM-TRAN.  Only if there is no other previous definition of the variable will it be understood to be an element of G or
 H.
...
list1  is  8;  otherwise,  it  is PDT.  (But see the NOAPPEND option.)
...
*** IDFORMAT=s(NM75)
      By default the ID column has the same format as specified by FORMAT.   IDFORMAT  specifies  the format for the ID column in table
      ...
      IDFORMAT=F6.1
           Floating value, with single digit to the right of the  decimal.
...
*** FIXEDETAS=(list)(NM74)
      ...
      groups of subjects, as if they were a fixed effect when  evaluating  populations  characteristics during the $TABLE step, such as
      PRED, CWRES, NPDE, etc. In this way, the PRED evaluated will  be,
      not  of  the total population, but of a given site level for that
      subject.  List is a number-list of ETAs.   For  example,  FIXEDETAS=(3-6)

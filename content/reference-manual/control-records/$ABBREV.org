...
*** COMRES=n1
Stands for "common reserve", one use of the option COMRES involves the presence of both abbreviated
and user-supplied subroutines. Values for variables defined in
abbreviated codes may be displayed in tables and scatterplots. These
variables are listed (i.e. their values are stored) in a NONMEM MODULE
NMPRD4 in the generated subroutine. (With versions of NONMEM prior to
nm7, NMPRD4 was a named FORTRAN common, hence the use of options
"COM..."). User-supplied FORTRAN routines may also list variables
whose values are to be displayed in NMPRD4.  The first \(n_1\)
positions in NMPRD4 are reserved for the use of these routines;
generated subroutines will not list variables defined in abbreviated
codes in these positions.  If there are no abbreviated codes, not only
is =COMRES= not needed for this purpose, it may not be used. All
variables listed in NMPRD4 must be double precision. .\"variables if
double precision NONMEM is used; otherwise, they must be .\"single
precision variables. The number \(\text{n}_{1}\) can be nonnegative,
and if the option is not used, it is understood to be 0.  It can also
be -1, in which case no variables defined in abbreviated codes are
listed in NMPRD4.
...
The option COMSAV ("common save") may be used when the variable COMACT is used in
abbreviated code. It defines the SAVE region of NMPRD4.
In this case the option COMRES should also be used, and
\(n_2\) must satisfy \( 0 \le n_2 \le n_1\).

- \(n_1=-1\): do not store any variables in the global variable array, VRBL, in the NMPRD4 module.

- \(n_1=0\): store  variables  in  NMPRD4  with  no  reserved  positions  (the default).

- \(n_1>0\): store variables in NMPRD4, but reserve the first \(n_1\) positions.
...
***  COMSAV= n2
      ...
      (See [[file:/docs/reference-manual/abbr/#copying-block][copying block]]).
...
***  DERIV1=NO (NM74)
Prevents the computation of first derivatives.  For example,  may be
used when only SAEM or BAYES is performed, or IMPMAP/ITS/FOCE are
performed using OPTMAP>0 and ETADER>0.  The  global  variable
...
:END:
~DERIV2=NO~ prevents  the computation of second-order derivatives, which are needed
...
they  cannot  be displayed in tables and scatterplots.  In addition,
...
(See [[file:/docs/reference-manual/api/variables/#partial-derivative-indicators][Partial Derivative Indicators]]).
...
REPLACE left_string = right_string
#+end_src
There are several different forms of the REPLACE option.  They do not affect verbatim code.  Case is ignored.
...
May  be  used  in  all  blocks of abbreviated code and $TABLE and
      ...
      With  this feature, indices of THETA, ETA, and EPS and ERR may
      be given symbolic names in abbreviated code, and $ABBR REPLACE is
      used  to  replace them with integer indices.  With NONMEM 7.4,
      ...
      *.ext,  ,phi,  etc., to maintain their third party software readability.
      ...
      See also [[file:/docs/reference-manual/control-records/model/][$MODEL]].
...
#+end_src
      ...
      (See [[file:/docs/reference-manual/abbr/#abbreviated-function][abbreviated functions]]).

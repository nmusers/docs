...
** Discussion
Optional. It is placed
before ~$PK~, ~$ERROR~, or ~$PRED~ abbreviated code blocks to signal
code storage, processing, aliasing, and declaration.

** Options

*** COMRES=n1
~COMRES~ stands for "common reserve". ~n1~ is the number of variables
reserved for user-supplied subroutines and verbatim code, in
NONMEM's variable storage module ([[file:/docs/reference-manual/api/modules/#headline-1][NMPRD4]]). In other words, when both abbreviated code and
user-supplied subroutines are used in a control
stream, the user-supplied subroutine has the first ~n1~ positions in
the storage module, and the first position for abbreviated code is
~n1+1~. Default ~n1=0~ (no reserved storage).

- ~n1=-1~: no space reserved for variables in abbreviated
  ...
  block, say ~$PK~, becomes inaccessible in other code blocks such as ~$ERROR~.

- Do not use this option when there are no abbreviated codes.

- Abbreviated code, verbatim code, ~$TABLE~,  and  ~$SCATTER~  may
  explicitly reference reserved variables
  using ~COM(I)~, ~I~ between 1 and ~n1~. For example,
  #+begin_src perl
    $TABLE COM(1) COM(2)
  #+end_src
...
copied from NMPRD4 in a [[file:/docs/reference-manual/abbr/#copying-block][/copying pass/]]. Normally with a given data

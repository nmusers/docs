...
** Discussion
 ...
 For a given eta, the points of support are the vector of posthoc estimates of that eta for all individuals (i.e., the CPE values  for  that
...
ETAS: Requests that conditional (nonparametric) estimates of eta
values be obtained. (Also called the CNPE).
...
*** NPESTIM=[0|1]
      The  default  non-parametric estimation method for assessing support point probabilities is an (non-Monte Carlo) expectation-maximization  (EM)  method.   You  may  also choose the non-negative
      least squares method (NNL, [29]), with NPESTIM=1.   While  it  is
      touted  to  be  faster  than  EM (NNL is quadratically convergent
      whereas EM is linearly convergent), several tests have not  indicated  that  NNL  is  any strong speed advantage.  The reason is,

...
With NONMEM 7.3 (value,value...)xn is permitted, so that repeated
      ...
      group of initial values may be enclosed in parentheses  and  followed  by "xn", which means to replicate the values within parentheses n times ("repeated value").

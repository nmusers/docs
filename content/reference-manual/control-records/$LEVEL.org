...
- nk[mk]: states  that  ETA(nk)  is associated with this super ID
  item, and ETA(mk) is nested within ETA(nk).
...
#+end_src
TO is required.  The character : may be used instead of  TO.   BY is optional. Default is 1.  The value of BY may be negative. If  the  second  value of BY differs, the same syntax may also be used
 for mk.
...
The  data  item  named SID is the site ID.  The data item named CID is
 ...
 eta(9) is the country variability that has nested in it the site variability eta(5), which in turn has nested in it the subject variability
 (the standard ID data) eta(1).  For V1, eta(10) is the  country  variability  that  has  nested in it the site variability eta(6), which in
...
See  also @@html:<a href="/docs/reference-manual/control-records/estimation/#levwt">LEVWT option</a>@@ of the $ESTIMATION record (NM74).  By default,

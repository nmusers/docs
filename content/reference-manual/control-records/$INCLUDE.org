...
* $INCLUDE (INCLUDE)
Causes NM-TRAN to read control stream records from a different file.
...
- filename:
      ...
      embedded spaces.  If it contains commas, semicolons, or parentheses, then it must be surrounded by  single  quotes  '  or  double
...
NMTRAN opens the named file and reads it to end-of-file.  The contents
 ...
 and re-reads it the specified number of times.  After reaching end-of-file  on the final (or only) copy, NMTRAN resumes reading the original
...
The file data3def contains the ~$INPUT~ and ~$DATA~ statements.  The  file modela  contains  the  ~$SUBROUTINE~  statement,  ~$PK~  block, and ~$ERROR~ block.

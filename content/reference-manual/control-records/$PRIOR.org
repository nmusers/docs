...
information: TNPRI uses a previous run's [[file:/docs/reference-manual/files/#msf][MSF]], and NWPRI uses
user-specified distribution hyperparameters. NONMEM7's new ~$ESTIMATION~ methods (BAYES, NUTS, SAEM, etc) can only use ~NWPRI~ to specify priors.

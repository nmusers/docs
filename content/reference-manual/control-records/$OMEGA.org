...
** DISCUSSION
This record gives initial estimates and constraints for elements of one or several
...
by "xn", which means to replicate the values within  parentheses n
...
#+end_src
This  gives the initial estimates of all the elements of a nondiagonal ("full") block of OMEGA.  E.g.,
#+begin_src perl
  $OMEGA BLOCK(2) .04 .002 .12
#+end_src
Initial estimate of variance of eta(1) = .04; initial estimate of
covariance of eta(2), eta(1) = .002; initial estimate of variance of
eta(2) = .12.
...
The  SCALE is most useful when it is desired to scale an informed
      ...
      prior by (df-p-1)/df, where df=degrees of freedom,  and  p=dimension  of  omega  block.  Rather than always re-calculate by hand,

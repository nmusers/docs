...
These  are  called the informative forms of the $THETA record, for use
 ...
 records in the control stream in the correct order.  When the informative forms are used, the options of $PRIOR NWPRI need  not  be  specified.  However, if options are listed in $PRIOR NWPRI, then these values are chosen over what is surmised from  the  informatively  labeled

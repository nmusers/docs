...
** Variables
In NONMEM v7+ a user-defined variable can have a name consisting  of
1-20 (specified by [[file:/docs/reference-manual/api/modules/#sizes-fsizes-prsizes][constant SD in SIZES]]) letters (A-Z), numerals
(0-9), understore ("_"), and begins
with a letter. It cannot be one of the reserved variable names or keywords.
A user-defined variable is of type double precision real. See
"subscripts" section below on the use of integer variables.

A user-define variable is used as the left-hand quantities in
assignment.
...
an  $INPUT record may not appear on the left in an assignment
statement.
...
- Subscripts variables may be used within the block.  These  are  variables  that  have  been  declared arrays using the $ABBR statement, or subscripted reserved variables such as THETA.  Subscripts may be integer  expressions  using declared integer variables and integer values. E.g.
  #+begin_src perl
    $ABBR DECLARE X(10)

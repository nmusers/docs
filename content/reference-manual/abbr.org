...
See also storage and calling options below.
...
Conditions can use Fortran's boolean expressions and operators: ~.EQ.~
(~==~), ~.NE.~ (~/=~), ~.LE.~ (~<=~),  ~.GE.~ (~>=~),  ~.LT.~ (~<~),
~.GT.~ (~>~),  ~.AND.~, and ~.OR.~.

- Conditions may not include parentheses except RHS quantities allowed in assignment:
  #+begin_src f90
    ;! Valid:
    IF (Q.EQ.(R+C)/D) ...
...
- When testing  ~ICALL~ in ~$PRED~, ~$PK~, and ~$ERROR~ blocks a
  condition like  ~IF (ICALL.EQ.2)~, may not use ~ELSE~ branch.
...
- Repeated definition in a same branch is not permitted:
  #+begin_src f90
    ! invalid
    IF (condition) THEN
...
- Random variables (variables expressed by random effects) cannot be
  defined within nested conditionals. Additionally, random variables
  missed by conditionals assume zero values. Thus
  #+begin_src f90
    IF (WT.GT.0) TVK=THETA(1)*WT
  #+end_src
  implies the ~TVK~ with ~WT.LE.0~ retains the value set previously
  (since ~TVK~ does not depend on any ~ETA~), while
  #+begin_src f90
    IF (WT.GT.0) K=THETA(1)*WT*EXP(ETA(1))
  #+end_src
  would cause random variable ~K~ corresponding to ~WT.LE.0~ set to
  zero, and warning message from NM-TRAN. The proper way to encode the
  above scenario would be to explicitly cover all cases:
  #+begin_src f90
    IF (WT.GT.0) THEN
    ...
    ENDIF
  #+end_src
  so that the ~K~ with ~WT.LE.0~ retains the value set previously.

For example, to /manually/ encode in ~$PRED~ the recursive
one-compartment kinetics with a single
bolus dose, we can do
#+begin_src f90
  K=THETA(1)*EXP(ETA(1))
  IF (TIME.EQ.0) THEN
    OLDA=AMT
  ...
  T=TIME
#+end_src
Similarly, for multiple bolus doses, we can do
#+begin_src f90
  K=THETA(1)*EXP(ETA(1))
  IF (TIME.EQ.0) THEN
    A=AMT
...
be used in the abbreviated code (default to 999). For example ([[https://github.com/nmusers/docs/tree/master/examples/bivariate][examples/bivariate]]), one can
declare
#+begin_src perl
...
These options provide special instructions and must
appear at the beginning of a control record. We recommend enclose them in
parentheses, with multiple options separated with semicolons.

A common example (may be used in all records except ~$MIX~) is
#+begin_src f90
COMRES=-1
#+end_src
It specifies that all quantities  defined in abbreviated code are to
be stored locally instead of with global scope.

See CALLFL below for calling options example.

** Reserved variables
Entries for ~$PRED~, ~$PK~,  ~$ERROR~,  ~$DES~,
~$AES~, ~$AESINITIAL~, and ~$TOL~ describe the reserved variable names for
these abbreviated codes. User-defined variables cannot collide with reserved variable names
unless with ~COMRES=-1~.
...
#+begin_src perl
 CALLFL=n
#+end_src
specifies the /calling protocol/,  the behavior of calling PK, ERROR, and some ADVAN routines:
- In ~$PK~ abbreviated code: control when PREDPP calls the [[file:/docs/reference-manual/api/subroutines/#PK][PK routine]].

- In ~$ERROR~ abbreviated code: control when PREDPP calls the [[file:/docs/reference-manual/api/subroutines/#error][ERROR routine]].

- In ~$AESINITIAL~ abbreviated code: when the TIME data item is not
  used, it controls when PREDPP calls the [[file:/docs/reference-manual/control-records/subroutines/#advan9-15-17][ADVAN9/ADVAN15/ADVAN17 routine]].

The values of ~CALLFL~ and corresponding behavior:

- ~CALLFL=-2~: call  the  PK subroutine with every event record,
  including /implied/
  [[file:/docs/reference-manual/data/#addl][additional]] and [[file:/docs/reference-manual/control-records/pk/#alagn][lagged]] doses, and at [[file:/docs/reference-manual/api/variables/#mtime][model event times]].  It does  not
  affect ERROR, ADVAN9, ADVAN15, or ADVAN17 routines.

  Corresponding message in the output:
  #+begin_src
...
- ~CALLFL=-1~ (default): call the subroutine with every event record.
  ...
  Corresponding message in the NONMEM output:
  #+begin_src
  PK SUBROUTINE CALLED WITH EVERY EVENT RECORD
...
- ~CALLFL=0~
  + For the PK subroutine: If the data are population data, call the
    subroutine with the first event record of each individual record and with every
    event record thereafter that has ~TIME~ differerent from the
  ...
  + Not applicable to the ADVAN9/ADVAN15/ADVAN17 routine.

  The [[file:/docs/reference-manual/data/#call][CALL data item]] may be used with ~CALLFL=0~ to request calls  with additional event records.

- ~CALLFL=1~:
  + For  the  PK and ADVAN9/ADVAN15/ADVAN17 subroutines: if the
  ...
  The [[file:/docs/reference-manual/data/#call][CALL data item]] may be used with ~CALLFL=1~ to request calls  with additional event records.

In a block of abbreviated code, the ~CALLFL~ option
must precede all the other abbreviated code (except for
verbatim code or other storage & calling options). It may not be used conditionally.
...
Alternative to ~CALLFL=n~ specification, one can use calling protocol phrases:
#+begin_src perl
...
During specific “copying passes” data records are sent to PRED to
become accessible. A copying pass is indicated by a positive value of
COMACT. Copying passes occur during variable output, in $TABLE or $SCATTER.

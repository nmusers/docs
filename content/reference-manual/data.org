...
RATE  labels  PREDPP's dose rate (RATE) data item.  The dose rate data
 ...
 - >0:   The  dose  is  an infusion, and the value of RATE gives its
   rate. If this is not a steady-state infusion, the AMT data item
   must be positive also.  If this is a steady-state dose event
   record, both AMT and RATE may be 0; (See ss dose).
...
:END:
 ...
 - 1:    The  data record serves as a template record, aiding in the
   ...
   the $OMIT record.)  The average is a two-stage average; it is the
   average of within-indi- vidual averages.
...
:END:
 ...
 file named on the $DATA record is the NONMEM data set.  See NONMEM
 Users Guide, Part IV.)
 ...
 option of $DATA record. The REPL option and REPL_ data item are meant
 to be used with $SIMULA- TION or $DESIGN.

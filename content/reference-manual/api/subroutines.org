...
- ICALL=-1: the routine has been called for the [[file:/docs/user-guide/advanced/abbr_block/#pred_ignore_data_block][ PRED_IGNORE_DATA]] feature (NM75).  One  call per data record, at the start
  ...
  the [[file:/docs/reference-manual/control-records/data/#pred_ignore_data_option][PRED_IGNORE_DATA option]] of $DATA is used.  Otherwise, there are
...
- IDEF: stores values only when ICALL=1. Values may be stored in the following elements:
  ...
  + IDEF(1,3) describes whether PK performs  compartment
    ...
    The default used by PREDPP is IDEF(1,3)=-1.  However,  when  compartment  initialization is not implemented, then if IDEF(1,3) is
      ...
      (See [[file:/docs/reference-manual/api/variables/#state-vec-a][State Vector A]]).

...
* Reserved variables
FORTRAN  modules are used to communicate values between various
components of the NONMEM system.  They supplement the subroutine
arguments.
...
*** Additional record counters
#+begin_src f90
 USAGE:
      include nonmem_reserved_general
#+end_src
In  addition  to  the record counters NDREC and NIREC, NONMEM 7.4
provides additional record counters.  All except  IRECIDX   refers  to
a record number within the current individual record.

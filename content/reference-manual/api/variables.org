...
COMRES=n1 (option of $ABBREVIATED record): COMRES  ("common  reserve")
is  an  option  of  the $ABBREVIATED record.  It gives instructions to
...
COMSAV=n2 (option of $ABBREVIATED record): values of variables displayed  in  tables  and  scatterplots  are
...
#+end_src
- TEMPLT: this variable serves two different purposes. At ICALL=6, the
  template data record is stored in TEMPLT. When  MIX  is  called,
  the  first  data record of the individual record is stored in
  TEMPLT.
